...
// 代码块模型（可能是GO AST语义分块也可能是文本分块），包含关联的项目ID、文件ID、文件路径、符号名称、符号类型、起始行、结束行、代码块文本内容、内容哈希值、代码版本哈希值、嵌入向量ID以及创建时间和更新时间等信息
type CodeChunk struct {
	ID              uint      `gorm:"primaryKey"`                      // 一段代码切片的唯一标识符，类型为uint，作为主键使用。
