...
type AnalysisTask struct {
	...
	Status          string     `gorm:"type:varchar(16);not null;default:pending;index:idx_analysis_tasks_status_created_at,priority:1;check:status IN ('pending','running','succeeded','failed')"` // 任务状态，类型为字符串，长度为16个字符，不能为空，默认值为pending，并且在数据库中具有索引约束。
	...
	CreatedAt       time.Time  `gorm:"autoCreateTime;index:idx_analysis_tasks_status_created_at,priority:2"` // 任务创建时间，类型为time.Time。
